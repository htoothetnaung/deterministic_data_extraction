...
const TITLES: Record<string, { title: string; breadcrumb: string }> = {
  ...
  documents: { title: "Documents", breadcrumb: "Workflow" },
  "ocr-review": { title: "Extraction Review", breadcrumb: "Workflow" },
  templates: { title: "Extraction Templates", breadcrumb: "Workflow" },
  "apply-template": { title: "Apply Template", breadcrumb: "Workflow" },
  benchmarking: { title: "Benchmarking", breadcrumb: "Evaluation" },
};

export function AppShell({ children }: { children: React.ReactNode }) {
  const view = useNav((s) => s.view);
  const meta = TITLES[view] ?? { title: "ExtractIQ", breadcrumb: "" };
  ...
  return (
    <footer className="mt-auto border-t border-border/60 bg-background/60 px-4 py-4 sm:px-6 lg:px-8">
      <div className="flex flex-col items-center justify-between gap-2 text-xs text-muted-foreground sm:flex-row">
        <p>
          ExtractIQ — Deterministic AI document extraction &amp; benchmarking
          platform.
        </p>
