...
* **[production_pipeline.py](app/services/production_pipeline.py)**: Orchestrates the document ingestion pipeline, running quick parses for page counts, cascading deep OCR parsers (Mistral OCR), and indexing chunk vectors.
...
* **[worker.py](app/services/worker.py)**: Background queue processor daemon that polls tasks, invokes deep OCR parsing, and commits vector representations.
